...
async function conectar(intentos = 10) {
  for (let i = 1; i <= intentos; i++) {
    try {
      ...
      console.log(`[RabbitMQ-S3] Conectado → ${EXCHANGE}`);
      conn.on('error', () => { channel = null; });
      return channel;
    } catch {
      console.warn(`[RabbitMQ-S3] Intento ${i}/${intentos}. Reintentando en 3s...`);
      await new Promise(r => setTimeout(r, 3000));
    }
  }
  throw new Error('[RabbitMQ-S3] No se pudo conectar');
}
...
async function publicar(routingKey, data) {
  ...
  console.log(`[RabbitMQ-S3] Publicado → ${routingKey}`);
}
...
async function consumir(routingKey, queueName, handler) {
  ...
  channel.consume(queue, async (msg) => {
    ...
    try {
      ...
      console.log(`[RabbitMQ-S3] Recibido ← ${routingKey}`);
      await handler(data);
      channel.ack(msg);
    } catch (err) {
      console.error('[RabbitMQ-S3] Error:', err.message);
      channel.nack(msg, false, false);
    }
  });
  console.log(`[RabbitMQ-S3] Escuchando "${routingKey}"`);
}
